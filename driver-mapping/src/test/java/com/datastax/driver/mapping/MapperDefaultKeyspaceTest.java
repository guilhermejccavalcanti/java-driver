...
public class MapperDefaultKeyspaceTest extends CCMTestsSupport {
    ...
    public void onTestContextInitialized() {
        execute(String.format("CREATE KEYSPACE IF NOT EXISTS %s WITH replication = { 'class' : 'SimpleStrategy', 'replication_factor' : 1 }", KEYSPACE), String.format("CREATE TYPE IF NOT EXISTS %s.group_name (name text)", KEYSPACE), String.format("CREATE TABLE IF NOT EXISTS %s.groups (group_id uuid PRIMARY KEY, name text)", KEYSPACE), String.format("CREATE TABLE IF NOT EXISTS %s.groups2 (group_id uuid PRIMARY KEY, name frozen<group_name>)", KEYSPACE));
    }
    ...
    @Test(groups = "short", expectedExceptions = IllegalArgumentException.class, expectedExceptionsMessageRegExp = "Error creating mapper for class com.datastax.driver.mapping.MapperDefaultKeyspaceTest\\$Group, the @Table annotation declares no default keyspace, and the session is not currently logged to any keyspace")
    ...
    @Test(groups = "short", expectedExceptions = IllegalArgumentException.class, expectedExceptionsMessageRegExp = "Error creating UDT codec for class com.datastax.driver.mapping.MapperDefaultKeyspaceTest\\$GroupName, the @UDT annotation declares no default keyspace, and the session is not currently logged to any keyspace")
    ...
    public static class Group {
        ...
        public boolean equals(Object other) {
            ...
            return Objects.equal(groupId, that.groupId) && Objects.equal(name, that.name);
        }
